...
var calendar = new FullCalendar.Calendar(calendarEl, {
  ...
  headerToolbar: {
    left: 'prev,next today,addScheduleBtn,selectTeamBtn,selectPrivate',
    center: 'title',
  ...
  eventClick: async function (arg) {
    ...
    if (url === ''){
      ...
      await $("#modifyBtn").off().on("click", () => {
        ...
        console.log("modify pList", pList)
          console.log("modify allList", allList)
      })
      ...
      $("#deleteBtn").off().on("click", () => {
        if (confirm('정말 삭제하시겠습니까?')) {
          ...
          pList = pList.filter((data) => data.id != eventDict.id)
          allList = allList.filter((data) => data.id != eventDict.id)
          console.log("del pList", pList)
          console.log("del allList", allList)
        }
      })
    }
  ...
  eventDrop: function (info) {
    console.log("infooo", info.oldEvent)
    oldEvent = {
      id: info.oldEvent.id,
  ...
  eventSources: [
    {
      events: async function () {
        ...
        await fetch(`/calendar/eventList`)
          ...
          .then(data => {
            console.log(data); // 서버에서 전달된 데이터 출력
            data.forEach((d) => {
              let dict = {}
              dict.id = `${d.CAL_ID}`
              dict.title = d.TITLE
