    ...
    setter:async(text)=>{
      ...
      console.log("textdao userName:",text.userName)
        ...
        if(result1.rows.length === 0){
         sql=`INSERT INTO chattingTable(indexChat, id, content,pdate)
            VALUES  ('1' ,'${text.userName}','${text.text}',sysdate)
            `; 
        }else{
         sql=`INSERT INTO chattingTable(indexChat, id, content,pdate)
        VALUES  ((select max(indexChat) from chattingTable)+1 ,'${text.ID}','${text.CONTENT}',sysdate)
        `;
        }
        const result =await(await con).execute(sql);
